...
//     @wire(fetchLookupData, { searchKey: '$searchKey', sObjectApiName: '$sObjectApiName', userType: '$selectedType' })
//      searchResult(value) {
...
// handelselectedType(event) { 
//     this.selectedType= event.detail
//     this.template.querySelector('c-custom-lookup-lwc').fetchLookupData();
//     console.log('UserTyp:'+this.selectedType);  
// }
    ...
    if (this.defaultRecordId !== '') {
        fetchDefaultRecord({ recordId: this.defaultRecordId, 'sObjectApiName': this.sObjectApiName })
            .then((result) => {
                if (result !== null) {
                    this.selectedRecord = result;
                    this.handelSelectRecordHelper(); // fonction d'aide pour afficher/masquer le conteneur de résultats de recherche sur l'interface utilisateur
                }
            })
            .catch((error) => {
                this.error = error;
...
//     this.fetchLookupData(); // Appelez votre fonction fetchLookupData ici
